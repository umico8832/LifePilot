...
- 状态：done
- 所属阶段：Phase 23
- 目标：为家庭空间提供不依赖真实邮件或短信服务的邀请链接能力，让管理员可以生成邀请，受邀已注册用户登录后接受加入空间。
- 涉及文件/模块：`backend/src/main/java/com/lifepilot/space/`、`backend/src/main/resources/db/migration/`、`backend/src/test/java/com/lifepilot/space/`、`frontend/src/api/space.ts`、`frontend/src/views/space/SpaceView.vue`、`docs/API_DESIGN.md`、`docs/DB_DESIGN.md`
- 验收标准：
  1. 新增邀请数据模型，记录空间、邀请人、目标邮箱（可选）、角色、token hash、状态、过期时间、接受时间。
  2. 后端提供创建邀请、列出空间邀请、撤销邀请、接受邀请接口；创建/撤销仅 `owner` 或 `admin` 可用。
  3. 接受邀请时要求用户已登录；邀请过期、撤销、已接受、目标邮箱不匹配或角色非法时返回统一错误结构。
  4. 不接真实邮件、短信或外部账号服务；前端只展示可复制的本地邀请链接或 token。
  5. 后端测试覆盖管理员创建、普通成员禁止、非成员禁止、过期/撤销/重复接受和接受后成为成员。
- 验证命令：`cd backend && ./mvnw test -B -Dtest=HouseholdControllerTests`；`cd backend && ./mvnw test -B`；`cd frontend && npm test`；`cd frontend && npm run build`；`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/API_DESIGN.md`、`docs/DB_DESIGN.md`、`docs/ARCHITECTURE.md`
- 是否阻塞后续任务：否
- 完成后建议下一任务：P9-002

### P9-002 前端邀请管理与接受邀请体验

- 优先级：P9
- 状态：done
- 所属阶段：Phase 23
- 目标：把 P9-001 的邀请接口接入空间管理页，让管理员能创建、复制、查看和撤销邀请，受邀用户能通过前端流程接受邀请。
- 涉及文件/模块：`frontend/src/api/space.ts`、`frontend/src/stores/space.ts`、`frontend/src/views/space/SpaceView.vue`、`frontend/src/router/index.ts`、`frontend/src/views/__tests__/`、`frontend/src/api/__tests__/space.test.ts`
- 验收标准：
  1. 空间管理页展示待处理邀请列表，包含角色、目标邮箱、过期时间、状态和撤销操作。
  2. 管理员可创建邀请并复制本地邀请链接；普通成员只读且不能看到管理操作。
  3. 新增接受邀请入口或页面，登录用户输入/打开 token 后可加入空间，并刷新空间列表。
  4. 邀请过期、撤销、邮箱不匹配和无权限时有明确错误态。
  5. 前端 API 或页面测试覆盖创建、撤销、接受邀请和只读状态；构建通过。
- 验证命令：`cd frontend && npm test`；`cd frontend && npm run build`；`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/API_DESIGN.md`（如路由或参数说明变化）
- 是否阻塞后续任务：否
- 完成后建议下一任务：P9-003

### P9-003 角色权限体验和测试矩阵加固

- 优先级：P9
- 状态：done
- 所属阶段：Phase 24
- 目标：梳理 `viewer`、`member`、`admin`、`owner` 在核心模块中的读写边界，让后端权限和前端操作可见性保持一致。
- 涉及文件/模块：`backend/src/main/java/com/lifepilot/**`、`backend/src/test/java/com/lifepilot/**`、`frontend/src/views/**`、`frontend/src/stores/space.ts`、`docs/API_DESIGN.md`、`docs/ARCHITECTURE.md`
- 验收标准：
  1. 文档明确各角色在空间、记账、购物、库存、待办、菜谱、饮食计划、票据、AI 日志中的读写能力。
  2. 后端至少补齐空间成员管理和一个业务模块的 viewer/member/admin 权限测试，防止只靠前端隐藏按钮。
  3. 前端关键页面根据角色隐藏或禁用不可用操作，并显示统一权限错误提示。
  4. 不弱化既有空间成员校验，不引入跨空间读取风险。
  5. 后端测试、前端测试和构建通过。
- 验证命令：`cd backend && ./mvnw test -B`；`cd frontend && npm test`；`cd frontend && npm run build`；`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/API_DESIGN.md`、`docs/ARCHITECTURE.md`
- 是否阻塞后续任务：否
- 完成后建议下一任务：P9-004

### P9-004 Demo seed 真实 MySQL 冒烟验证与浏览器检查

- 优先级：P9
- 状态：done
- 所属阶段：Phase 24
- 目标：在本地 MySQL 可用时补跑 demo seed 的真实写入、幂等验证和主要页面冒烟检查，让演示体验从脚本 dry-run 进入可验证状态。
- 涉及文件/模块：`scripts/demo_seed.sh`、`scripts/demo_seed.sql`、`docs/HANDOFF.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、必要时 `agent-skills/lifepilot-webapp-testing/`
- 验收标准：
  1. 本地 MySQL 可用后执行 `MYSQL_PORT=3307 scripts/demo_seed.sh --apply` 和 `--verify` 成功。
  2. 重复执行 `--apply` 后记录计数稳定，不产生不可控重复数据。
  3. 使用 demo 账号登录前端，检查首页、购物清单、库存提醒、饮食计划和 AI 日志页面有可见数据。
  4. 若 Docker 或浏览器工具不可用，必须在 changelog 和 CURRENT_STATE 中记录原因、替代验证和后续补跑命令。
  5. 不提交真实 `.env` 或真实个人数据。
- 验证命令：`MYSQL_PORT=3307 scripts/demo_seed.sh --apply`；`MYSQL_PORT=3307 scripts/demo_seed.sh --verify`；`cd backend && ./mvnw test -B`；`cd frontend && npm run build`；`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/HANDOFF.md`（如运行步骤变化）
- 是否阻塞后续任务：否
- 完成后建议下一任务：待定
...
- 状态：todo
- 所属阶段：Phase 25
- 目标：按 P9-003 角色矩阵补齐购物清单后端写权限和前端只读体验，确保 viewer 只能查看购物清单和清单项。
- 涉及文件/模块：`backend/src/main/java/com/lifepilot/shopping/`、`backend/src/test/java/com/lifepilot/shopping/`、`frontend/src/views/shopping/ShoppingView.vue`、`frontend/src/views/__tests__/`、`docs/API_DESIGN.md`、`docs/ARCHITECTURE.md`
- 验收标准：
  1. 后端购物清单创建、更新、删除、清单项新增/更新/删除要求 `owner/admin/member`，viewer 可读取但不可写。
  2. 后端测试覆盖 viewer 可读不可写、member/admin 可写、非成员禁止。
  3. 前端购物清单页在 viewer 角色下隐藏或禁用新增、编辑、删除、采购状态变更等写操作，并展示只读提示。
  4. 不弱化既有空间成员隔离和业务校验。
  5. 后端测试、前端测试和构建通过。
- 验证命令：`cd backend && ./mvnw test -B -Dtest=ShoppingControllerTests,ShoppingServiceTests`；`cd frontend && npm test -- ShoppingView.test.ts`；`cd backend && ./mvnw test -B`；`cd frontend && npm test`；`cd frontend && npm run build`；`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/API_DESIGN.md`、`docs/ARCHITECTURE.md`
- 是否阻塞后续任务：否
- 完成后建议下一任务：P10-002

### P10-002 库存与待办 viewer 写保护加固

- 优先级：P10
- 状态：todo
- 所属阶段：Phase 25
- 目标：按角色矩阵补齐库存和待办模块的 viewer 只读保护，让家庭协作中的普通维护和只读观察边界清晰。
- 涉及文件/模块：`backend/src/main/java/com/lifepilot/inventory/`、`backend/src/main/java/com/lifepilot/todo/`、`backend/src/test/java/com/lifepilot/inventory/`、`backend/src/test/java/com/lifepilot/todo/`、`frontend/src/views/inventory/InventoryView.vue`、`frontend/src/views/todo/TodoView.vue`、`docs/API_DESIGN.md`
- 验收标准：
  1. 后端库存和待办写操作要求 `owner/admin/member`，viewer 可读取但不可新增、编辑、删除或完成任务。
  2. 后端测试覆盖 viewer 可读不可写和 member/admin 可写。
  3. 前端库存和待办页面在 viewer 角色下隐藏或禁用写操作，并展示只读提示。
  4. 不改变统计、提醒和只读列表的访问能力。
  5. 后端测试、前端测试和构建通过。
- 验证命令：`cd backend && ./mvnw test -B -Dtest=InventoryControllerTests,TodoControllerTests,InventoryServiceTests,TodoServiceTests`；`cd frontend && npm test`；`cd frontend && npm run build`；`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/API_DESIGN.md`
- 是否阻塞后续任务：否
- 完成后建议下一任务：P10-003

### P10-003 菜谱、饮食计划与票据 viewer 写保护加固

- 优先级：P10
- 状态：todo
- 所属阶段：Phase 25
- 目标：将角色矩阵落实到菜谱、饮食计划和票据文档模块，完成主要生活数据模块的 viewer 只读后端保护。
- 涉及文件/模块：`backend/src/main/java/com/lifepilot/recipe/`、`backend/src/main/java/com/lifepilot/document/`、`backend/src/test/java/com/lifepilot/recipe/`、`backend/src/test/java/com/lifepilot/document/`、`frontend/src/views/recipe/RecipeView.vue`、`frontend/src/views/mealplan/MealPlanView.vue`、`frontend/src/views/document/DocumentView.vue`、`docs/API_DESIGN.md`
- 验收标准：
  1. 后端菜谱、饮食计划和票据文档写操作要求 `owner/admin/member`，viewer 可读取但不可写。
  2. 后端测试覆盖至少每个模块的 viewer 写入禁止和 member/admin 写入允许。
  3. 前端相关页面在 viewer 角色下隐藏或禁用写操作，并展示只读提示。
  4. AI 推荐和草稿仍只生成建议，不绕过业务写权限。
  5. 后端测试、前端测试和构建通过。
- 验证命令：`cd backend && ./mvnw test -B -Dtest=RecipeControllerTests,MealPlanServiceTests,DocumentControllerTests`；`cd frontend && npm test`；`cd frontend && npm run build`；`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/API_DESIGN.md`
- 是否阻塞后续任务：否
- 完成后建议下一任务：P10-004

### P10-004 权限矩阵回归与演示路径整理

- 优先级：P10
- 状态：todo
- 所属阶段：Phase 26
- 目标：在主要模块 viewer 写保护落地后，整理一套可重复的权限回归检查和演示路径，便于后续真实浏览器/数据库环境可用时快速补跑。
- 涉及文件/模块：`docs/TESTING.md`、`docs/HANDOFF.md`、`docs/API_DESIGN.md`、`docs/CURRENT_STATE.md`、必要时 `scripts/`
- 验收标准：
  1. 文档给出 owner/admin/member/viewer 权限回归清单，覆盖空间、记账、购物、库存、待办、菜谱、饮食计划、票据和 AI 日志。
  2. HANDOFF 或 TESTING 记录 demo seed、角色账号准备和浏览器冒烟的建议顺序。
  3. 若仍缺真实 MySQL 或浏览器工具，记录准确原因和后续补跑命令。
  4. 文档一致性检查通过。
- 验证命令：`python3 scripts/agent_changelog_archive.py`；`python3 scripts/agent_doc_check.py`；`git diff --check`
- 完成后更新：`docs/BACKLOG.md`、`docs/CURRENT_STATE.md`、`docs/CHANGELOG_AGENT.md`、`docs/TESTING.md`、`docs/HANDOFF.md`
- 是否阻塞后续任务：否
- 完成后建议下一任务：待定
