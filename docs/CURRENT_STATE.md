...
- 本次本地验证：`cd backend && ./mvnw test -B` 通过，240 tests passed；`cd frontend && npm ci` 通过；`cd frontend && npm test` 通过，11 个测试文件 92 tests passed；`cd frontend && npm run build` 通过，已通过构建阈值配置消除大 chunk 警告，仍有第三方 `@vueuse/core` Rolldown pure annotation 警告；`cd frontend && npm audit --audit-level=high` 通过；`cd frontend && npm run lint` 失败，原因是项目尚未定义 lint 脚本。
